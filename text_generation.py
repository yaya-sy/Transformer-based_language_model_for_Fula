...
def nucleus_sampling(model, device, data_generator, prompted, temperature=0.95, top_k=15, top_p=0.90, max_predicted_units=64) :
    with torch.no_grad() :
        gen_idxs = []
        gen_idxs += prompted
        end = False
        for _ in range(max_predicted_units) :
            if end : continue
            inputs = torch.tensor(gen_idxs).view(1, -1).to(device)
            logits = model(inputs)
            logits = logits[0, -1, :]
        ...
        return data_generator.decode(gen_idxs)